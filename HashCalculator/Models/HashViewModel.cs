...
internal class HashViewModel : INotifyPropertyChanged
{
    ...
    private readonly object cmpResultLock = new();
    private readonly ManualResetEvent pauseManualResetEvent = new(true);
    private readonly object manipulationLock = new();
    ...
    public void ComputeManyHashValue()
    {
        ...
        {
            case AlgoType.SHA1:
                algoObject = SHA1.Create();
                break;
            case AlgoType.SHA384:
                algoObject = SHA384.Create();
                break;
            case AlgoType.SHA512:
                algoObject = SHA512.Create();
                break;
            case AlgoType.MD5:
                algoObject = MD5.Create();
                break;
    ...
    private void UsingBouncyCastleSha224(Stopwatch stopwatch)
    {
        ...
        try
        {
            using (FileStream fs = File.OpenRead(this.Path.FullName))
            {
                ...
                int readedSize = 0;
                Sha224Digest algorithmHash = new();
                byte[] buffer = new byte[blockSize];
                while (true)
                {
                    if (this.token.IsCancellationRequested)
    ...
    public bool StartupModel(bool force)
    {
        lock (manipulationLock)
        {
                ...
                this.Result != HashResult.Succeeded))
            {
                ...
                this.PrepareToken();
                this.ModelCanbeStartedEvent?.Invoke(this);
                return true;
            }
            return false;
        }
